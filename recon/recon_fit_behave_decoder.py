...
class ReconFitterBehave(ReconFitterBase):
    def fit_recon(self, args):
        ...
        # prepare model
        model = ACTIONCHORE_decoder(args)
        # generator
    ...
    def optimize_smpl_object(self, model, data_dict, obj_iter=20, joint_iter=10, steps_per_iter=10):
        ...
        data_dict['smpl_center'] = self.compute_smpl_center_pred_decoder(data_dict, model, smpl)
    ...
    def forward_step(self, model, smpl, data_dict, obj_R, obj_t, obj_s, phase):
        ...
        df_pred, _, _, centers_pred_o, _ = preds
        ...
        if phase == 'sil':
            ...
            if phase == 'joint':
                ...
                preds = model.get_preds()
                df_pred, centers_pred_h = preds[0], preds[-2]
    ...
    def forward_smpl(self, smpl, data_dict, phase):
        ...
        smpl_verts, _, _, _ = smpl()
        df_pred, parts_pred, centers_pred = self.compute_df_h_loss_decoder(data_dict, loss_dict, model, smpl_verts)
